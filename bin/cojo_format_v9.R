...
# if A2 is missing in the raw gwas, use the non_effect_allele in snp.info 
...
####################################################################################
# put in p value column

valid_p_values <- gwas[, colname.p]
valid_p_values <- valid_p_values[is.finite(valid_p_values)]


if ( max(valid_p_values, na.rm = TRUE) < 1.1 ){

formatted.gwas$p = as.numeric(gwas[,colname.p])
}else{
formatted.gwas$p = 10^(-gwas[,colname.p])
}
...
formatted.gwas = formatted.gwas[which(formatted.gwas$b != 0),]
formatted.gwas = formatted.gwas[which(formatted.gwas$se != 0),]
