...
gwas = data.frame(fread(gwas.file))
predictor = data.frame(fread(predictor.file))
...
#predictor$gwas.se =  gwas[match(predictor$Name, gwas$SNP),"se"]
#predictor$threshold = abs(predictor$effect.in.gwas) + 3*(predictor$gwas.se)
#predictor$outlier = (abs(predictor$A1Effect) > abs(predictor$threshold))

st=format(Sys.time(), "%Y%m%d_%H_%M")

#write.table(predictor[which(predictor$outlier == TRUE),], paste0("Figures/", trait, "_effect_size_outliers.txt"  ), quote = F, sep ="\t"  )

effect.plot = ggplot(data = predictor, aes(x = marginal.effect, y = A1Effect  
					   # , color= outlier
					   )) + geom_point(size = 0.4) +  
  geom_abline(intercept=0, slope=1, color="blue")    +
  labs(title=trait,  x="GWAS marginal effect", y = "SBayesR Effect size") 


ggsave(effect.plot, filename = paste0(predictor.file,  "_compare_marginal_effect_vs_SBayesRC_", st, ".png"), height = 8, width = 8)
