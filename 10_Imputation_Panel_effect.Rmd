---
title: "PGS from a same genotyping using different imputation panels"
output:
  html_document:
    toc: TRUE
    toc_float: TRUE
...
We selected 115 traits to do the analysis. Each analysis will take some of them as examples to put in the main figure, and separate the last of them into 5 groups to put in sup figures.
...
We used the European samples in the batch 1 of Lifelines data. The data was imputed using HRC imputation panel in house.
...
Here is a plot of average PGS comparing between the two imputation panel of 
...
# Fig 3. Variation from imputation panels. 

We will use QIMR data to show it, since they are imputed in both panels. 
...
example.impu.hist = ggarrange(example.impu.hist1, example.impu.hist2, nrow =2)
#ggsave(example.impu.hist, filename = "Figures/Fig_PGS_benchmarking_example_HRC_vs_TOPmed.jpeg", width = 12, height = 5)

```



# Sup Fig 3. all traits
...
hist(cross.sample.mean$diff_from_convert)
```
...
# ggsave(example.convert.hist, filename = "Figures/Fig4_PGS_consistency_when_using_converted_predictors_in_ToPMed.jpeg", width = 12, height = 3)
```
